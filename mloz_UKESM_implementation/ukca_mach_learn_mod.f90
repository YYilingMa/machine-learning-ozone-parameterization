...
SUBROUTINE ukca_mach_learn(row_length, rows, model_levels, temperature, ml_ozone)
...
REAL,    INTENT(INOUT) :: ml_ozone(row_length, rows, model_levels)
...
IF (l_first) THEN
   ...
   IF (.NOT. ALLOCATED(daily_mean_temp))                                        &
        ALLOCATE(daily_mean_temp(1:row_length, 1:rows, 1:model_levels))
   daily_mean_temp(:,:,:) = 0.0

   ! currently number of coeffs is equal to model_levels. May need to be 
   ! namelist input eventually
   n_coeffs = model_levels
   IF (.NOT. ALLOCATED(ml_coeffs))                                              &
        ALLOCATE(ml_coeffs(1:row_length, 1:rows, 1:model_levels, 1:n_coeffs))
   ml_coeffs(:,:,:,:) = 0.0
   CALL get_ml_coeffs(row_length, rows)

   ! ozone scalings to convert to correct units
   IF (.NOT. ALLOCATED(ml_ozone_mean))                                          &
        ALLOCATE(ml_ozone_mean(1:row_length, 1:rows, 1:model_levels))
   ml_ozone_mean(:,:,:) = 0.0
   IF (.NOT. ALLOCATED(ml_ozone_scale))                                         &
        ALLOCATE(ml_ozone_scale(1:row_length, 1:rows, 1:model_levels))
   ml_ozone_scale(:,:,:) = 0.0
...
IF (current_day_number /= i_day_number) THEN
   ...
   ! will not change
   IF (.NOT. ALLOCATED(tvec)) ALLOCATE(tvec(1:model_levels))
   IF (.NOT. ALLOCATED(mlmat)) ALLOCATE(mlmat(1:model_levels,1:n_coeffs))
   IF (.NOT. ALLOCATED(ml_ozone_pred)) ALLOCATE(ml_ozone_pred(1:model_levels))
...
IF (current_day_number == i_day_number) THEN
   daily_mean_temp(:,:,:) = daily_mean_temp(:,:,:) + temperature(:,:,:)
   n_times = n_times + 1
END IF
...
USE ukca_option_mod, ONLY: ukca_ml_scaling
IMPLICIT NONE

INTEGER, INTENT(IN)  :: row_length, rows

CHARACTER(LEN=varname_len), PARAMETER :: var_mean ='y_mean'
CHARACTER(LEN=varname_len), PARAMETER :: var_scale='y_scale'
LOGICAL :: l_norec
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='UKCA_ML_SCALING'

IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_in,zhook_handle)

CALL ml_fopen(TRIM(ADJUSTL(ukca_ml_scaling)),fileid)

! y_out = y_pred*y_scale+y_mean

! mean of ozone
CALL ml_get_data (row_length, rows, fileid, TRIM(ADJUSTL(var_mean)),            &
                  ml_ozone_mean, l_norec)

! ozone scaling
CALL ml_get_data (row_length, rows, fileid, TRIM(ADJUSTL(var_scale)),           &
                  ml_ozone_scale, l_norec)
