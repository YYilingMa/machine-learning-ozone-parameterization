...
filepath = '/hkfs/work/workspace/scratch/ou4895-ukesm/output/RidgeModel_73yrsTrain_under50km_4CO2_76lev/'
...
# nn=0
# for i_batch in [0]:  #range(0,n_batch,1):
#     ### scaler_x
#     scaler_x = joblib.load(filepath+'Scaler_x_'+str_exp+'_lon_batch'+str(i_batch)+'.pkl')
#     x_scale = scaler_x.scale_.reshape((n_level,n_lat,n_proma)) 
#     x_mean = scaler_x.mean_.reshape((n_level,n_lat,n_proma)) 
#     x_means[:,:,nn*n_proma:(nn+1)*n_proma] = x_mean
#     x_scales[:,:,nn*n_proma:(nn+1)*n_proma] = x_scale

#     ### scaler_y
#     scaler_y = joblib.load(filepath+'Scaler_y_'+str_exp+'_lon_batch'+str(i_batch)+'.pkl')
#     y_scale = scaler_y.scale_.reshape((n_level,n_lat,n_proma)) 
#     y_mean = scaler_y.mean_.reshape((n_level,n_lat,n_proma)) 
#     y_means[:,:,nn*n_proma:(nn+1)*n_proma] = y_mean
#     y_scales[:,:,nn*n_proma:(nn+1)*n_proma] = y_scale
...
#         for levi in range(0,n_level):
#             for j in range(0,n_proma):
#                 coefs[:,levi,lati,nn*n_proma+j] = models['lon_'+str(j)+'_lat'+str(lati)+'_level'+str(levi)].coef_
#                 alphas[levi,lati,nn*n_proma+j] = models['lon_'+str(j)+'_lat'+str(lati)+'_level'+str(levi)].alpha
#     nn += 1
